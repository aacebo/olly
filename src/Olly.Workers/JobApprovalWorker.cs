...
public class JobApprovalWorker(IServiceProvider provider, IServiceScopeFactory scopeFactory) : IHostedService
{
    private ILogger<JobApprovalWorker> Logger { get; init; } = provider.GetRequiredService<ILogger<JobApprovalWorker>>();
    private NetMQQueue<JobApprovalEvent> Queue { get; init; } = provider.GetRequiredService<NetMQQueue<JobApprovalEvent>>();
    ...
    protected async Task OnStart(NetMQQueue<JobApprovalEvent> queue, CancellationToken cancellationToken)
    {
    ...
    protected Task OnEvent(JobApprovalEvent @event, CancellationToken _ = default)
    {
