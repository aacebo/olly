...
public class ChatWorker(IServiceProvider provider, IServiceScopeFactory scopeFactory) : IHostedService
{
    private ILogger<ChatWorker> Logger { get; init; } = provider.GetRequiredService<ILogger<ChatWorker>>();
    private NetMQQueue<ChatEvent> Queue { get; init; } = provider.GetRequiredService<NetMQQueue<ChatEvent>>();
    ...
    protected async Task OnStart(NetMQQueue<ChatEvent> queue, CancellationToken cancellationToken)
    {
    ...
    protected Task OnEvent(ChatEvent @event, CancellationToken _ = default)
    {
        if (@event.Chat.SourceType.IsTeams)
        {
            TeamsQueue.Enqueue(@event);
        }
        else if (@event.Chat.SourceType.IsGithub)
        {
