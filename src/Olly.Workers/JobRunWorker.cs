...
public class JobRunWorker(IServiceProvider provider, IServiceScopeFactory scopeFactory) : IHostedService
{
    private ILogger<JobRunWorker> Logger { get; init; } = provider.GetRequiredService<ILogger<JobRunWorker>>();
    private NetMQQueue<JobRunEvent> Queue { get; init; } = provider.GetRequiredService<NetMQQueue<JobRunEvent>>();
    ...
    protected async Task OnStart(NetMQQueue<JobRunEvent> queue, CancellationToken cancellationToken)
    {
    ...
    protected Task OnEvent(JobRunEvent @event, CancellationToken _ = default)
    {
